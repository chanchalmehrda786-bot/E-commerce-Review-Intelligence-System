"""
E-commerce Review Intelligence — app.py  v3.0
Fixes: sentiment all-neutral bug, large-file upload, blue animated sidebar,
lavender/purple dashboard palette (image-4 colours), CSS page-enter animations.
...
# ══════════════════════════════════════════════════════════════════════════════
#  GLOBAL CSS — blue sidebar + lavender main + animations
# ══════════════════════════════════════════════════════════════════════════════
st.markdown("""
<style>
...
/* ══ SIDEBAR — deep blue like reference image ══ */
...
div[data-testid="stRadio"] > div > label {
    ...
    cursor:pointer !important; font-size:13.5px !important; font-weight:500 !important;
...
/* ACTIVE — white pop-out with animation */
div[data-testid="stRadio"] > div > label:has(input:checked){
    background:#ffffff !important; color:#4f46e5 !important; font-weight:700 !important;
...
# Realistic diverse demo data (proper sentiment distribution)
_RAW = [
...
DEMO_REVIEWS = _RAW * 175   # ~7000 rows total


# ══════════════════════════════════════════════════════════════════════════════
#  NLP — FIXED: uses star rating + VADER compound together
# ══════════════════════════════════════════════════════════════════════════════
@st.cache_resource
def get_sia():
...
def classify_sentiment(text: str, stars=None) -> str:
    """Blend VADER score with star rating to avoid all-neutral problem."""
    ...
    try:
        ...
        else:  # 3 stars — genuine neutral zone
            if score >= 0.15:   return "Positive"
            elif score <= -0.15: return "Negative"
            else:                return "Neutral"
    except Exception:
        pass
    # VADER only (wider bands than default)
...
# ══════════════════════════════════════════════════════════════════════════════
#  DATA LOADER — handles any size file
# ══════════════════════════════════════════════════════════════════════════════
def load_data(source, local_path, uploaded_file, n_rows):
    ...
    try:
        if source == "Demo sample":
            ...
            raw = uploaded_file.read()           # read all bytes at once
            buf = io.BytesIO(raw)
...
with st.sidebar:
    st.markdown("""
    <div style="padding:18px 14px 14px;display:flex;align-items:center;gap:10px;">
      <div style="width:38px;height:38px;border-radius:10px;
                  background:rgba(255,255,255,.2);border:1px solid rgba(255,255,255,.3);
                  display:flex;align-items:center;justify-content:center;font-size:20px;">📊</div>
      <div>
        <div style="font-size:14px;font-weight:800;color:#fff;letter-spacing:.2px;">ReviewIQ</div>
        <div style="font-size:10px;color:rgba(255,255,255,.6);">E-commerce Intelligence</div>
      </div>
    </div>
    """, unsafe_allow_html=True)

    page = st.radio("nav",
        ["🏠  Overview","😊  Sentiment","⚠️  Fake Review",
         "🔺  Issues","📋  Reviews","📈  Trends",
         "📄  Reports","⚙️  Settings"],
        label_visibility="collapsed")

    st.markdown("""<hr style="border:none;border-top:1px solid rgba(255,255,255,.2);margin:8px 14px;">
    <div style="color:rgba(255,255,255,.5);font-size:11px;font-weight:700;
                letter-spacing:1.2px;text-transform:uppercase;padding:0 14px 6px;">DATA</div>
    """, unsafe_allow_html=True)
    ...
    if source == "Local dataset file":
        ...
        st.markdown("<div style='font-size:11px;color:rgba(255,255,255,.7);margin-bottom:4px;'>"
                    "CSV / JSON / ZIP (any size)</div>", unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    csv_b=df[["reviewText","overall","Sentiment","FakeReview","FakeScore","Issue","Date"]
             ].to_csv(index=False).encode("utf-8")
    c1,c2=st.columns(2)
    with c1:
        st.download_button("⬇️  Full Analysis CSV",csv_b,
    ...
    with c2:
        ...
        st.download_button("⬇️  Summary TXT","\n".join(lines).encode(),
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with c2:
        ...
        <div style="font-size:13px;color:#374151;line-height:1.9;">
          ...
          Version 3.0 &nbsp;|&nbsp; Streamlit + Plotly<br><br>
